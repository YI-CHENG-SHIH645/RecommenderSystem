...
double CF::test_rmse(int k, double simi_th) {
    ...
    SP UIMAT;
    if(SP::IsRowMajor) {
        std::sort(test_data.begin(), test_data.end(), [&reader_to_idx](const auto &a, const auto &b) {
            return reader_to_idx[std::get<0>(a)] < reader_to_idx[std::get<0>(b)];
        });
        UIMAT = input.train_data_row();
    }
    else {
        std::sort(test_data.begin(), test_data.end(), [&book_to_idx](const auto &a, const auto &b) {
    ...
    for(auto & r : test_data) {
        ...
        idx = SP::IsRowMajor ? reader_to_idx[reader] : book_to_idx.count(book) ? book_to_idx[book] : -1;
        another_idx = !SP::IsRowMajor ? reader_to_idx[reader] : book_to_idx.count(book) ? book_to_idx[book] : -1;
        if(idx == -1)
            pred_rating = 3;
        else if(idx != cur_idx) {
            cur_idx = idx;
